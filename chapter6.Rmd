...
title_meta  : บทที่ 6
title       : Lists
description : List นั้นไม่เหมือนกับ vector มันสามารถเก็บข้อมูลหลายประเภทไว้ด้วยกันได้ ในบทนี้เราจะสอนให้คุณรู้จักการสร้าง ตั้งชื่อ และเลือกข้อมูลจาก list!
...
- **Data frame** (2 มิติ): สามารถเก็บข้อมูลประเภท numeric, character หรือ logical ได้ ข้อมูลทุกตัวใน 1 คอลัมน์ต้องเป็นประเภทเดียวกัน แต่สามารถเก็บคอลัมน์คนละประเภทกันไว้คนละคอลัมน์ได้
...
**list** ใน R ก็เหมือนกับ to-do list ที่คุณทำเพื่อเตือนตัวเองว่าแต่ละวันต้องทำอะไรบ้าง เพราะใน list แต่ละอันอาจจะประกอบไปด้วยข้อมูลที่หลากหลายต่างประเภทกัน... 
...
หรือเราอาจสามารถพูดได้ว่า list นั้นเป็นข้อมูลแบบพิเศษแบบหนึ่ง ซึ่งเราสามารถเก็บอะไรไว้ข้างในก็ได้!
...
arguments ของ function `list()` คือส่วนประกอบที่คุณจะใส่ลงไปใน list จำไว้ว่าส่วนประกอบเหล่านี้สามารถเป็นได้ทั้ง matrices, vectors, หรือแม้กระทั่ง list อันอื่นๆ เป็นต้น
...
ในลำดับต่อไป คุณอาจจะไม่อยากจำว่า object แต่ละอย่างที่อยู่ใน list คืออะไรบ้าง ซึ่งคุณสามารถแก้ปัญหาโดยการตั้งชื่อให้กับมันได้ Just like on your to-do list, you want to avoid not knowing or remembering what the components of your list stand for. That is why you should give names to them: 
...
ด้วยคำสั่งนี้ คุณขะทำการสร้าง list ซึ่งมีชื่อเป็น `name1` และ `name2` แต่ในกรณีที่คุณอยากตั้งชื่อให้กับส่วนประกอบของ list หลังจากที่คุณได้สร้าง list ไปแล้ว คุณสามารถใช้ function `names()` ช่วยได้ เหมือนกับที่คุณทำกับ vector ลองดูโค้ดด้านล่างเป็นตัวอย่าง: If you want to name your lists after you've created them, you can use the `names()` function as you did with vectors. The following commands are fully equivalent to the assignment above:
...
อยู่มาวันหนึ่ง คุณก็เกิดอยากที่จะเก็บข้อมูลเกี่ยวกับภาพยนตร์ดีๆไว้ใน list ขึ้นมา

เริ่มจากการสร้าง list สำหรับเก็บข้อมูลเกี่ยวกับภาพยนตร์เรื่อง "The Shining" กันก่อน เราได้สร้างตัวแปร `mov`, `act` และ `rev` ไว้ให้คุณใน workspace แล้ว คุณสามารถลองเช็คค่าของตัวแปรเหล่านี้ได้ใน Console
...
list ของคุณมักจะประกอบด้วยส่วนประกอบหลายตัว ในกรณีที่คุณอยากจะดึงส่วนประกอบของมันออกมา ไม่ว่าจะตัวเดียวหรือหลายตัว คุณจะต้องใช้คำสั่งที่แตกต่างไปจากตอนที่ดึงข้อมูลจาก vector, matrix หรือ data frame เล็กน้อย

ทางหนึ่งในการเลือกส่วนประกอบจาก list คือการใช้เลขตำแหน่งของส่วนประกอบนั้นๆ ยกตัวอย่างเช่น ในการที่จะนำส่วนประกอบตัวแรกสุดของ `shining_list` ออกมา คุณต้องพิมพ์คำสั่งว่า: 

```
shining_list[[1]]
```

จะเห็นได้ว่าค่อนข้างคล้ายกับตอนที่คุณเลือกข้อมูลออกมาจาก vector คุณสามารถลองตรวจสอบผลลัพธ์ของคำสั่งดังกล่าวได้โดยการพิมพ์มันลงไปบน Console จุดที่แตกต่างกันก็คือ ในการเลือกข้อมูลจาก vector คุณใช้ square bracket (`[ ]`) แค่อันเดียวเท่านั้น

นอกจากนี้ คุณยังสามารถเลือกส่วนประกอบออกมาโดยการใช้ชื่อของส่วนประกอบนั้นๆได้ผ่านการใช้คำสั่ง `[[ ]]` หรือเครื่องหมาย `$` เช่นกัน คำสั่งทั้ง 2 คำสั่งต่อไปนี้จะทำการเลือก data frame ที่เก็บ review ของภาพยนตร์ไว้ออกมา: 
...
ในบางครั้ง คุณอาจจะต้องการเลือกข้อมูลออกมาจากส่วนประกอบแต่ละตัวอีกที ยกตัวอย่างเช่น ด้วยคำสั่ง `shining_list[[2]][1]` คุณจะทำการเลือกข้อมูลตัวแรก(`[1]`) ออกมาจากส่วนประกอบตัวที่ 2 หรือ `actors` (`shining_list[[2]]`) ถ้าคุณได้ลองพิมพ์คำสั่งนี้ลงไปบน Console คุณจะเห็นค่าที่แสดงออกมาเป็นชื่อ Jack Nicholson
