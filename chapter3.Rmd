...
ใน R matrix คือการเก็บข้อมูลประเภทเดียวกัน (numeric, character, หรือ logical) หลายๆตัวไว้ในรูปของตารางซึ่งประกอบไปด้วยแถวและคอลัมน์(ลองนึกภาพ vector หลายๆอันมาต่อกัน) และเนื่องจากเราสามารถระบุตำแหน่งของข้อมูลแต่ละตัวได้ด้วยเลขแถวและคอลัมน์ matrix จึงมี 2 มิติ! (และจะมีขนาดเท่ากับ จำนวนแถว * จำนวนคอลัมน์)
...
คุณสามารถตั้งชื่อให้กับแถวและคอลัมน์ของ matrix ได้ คล้ายๆกับตอนที่เราตั้งชื่อให้กับข้อมูลใน vector ลองดูคำสั่งด้านล่างเป็นตัวอย่าง:
...
สำหรับภาพยนตร์เรื่องใดๆก็ตาม สิ่งสำคัญส่วนหนึ่งที่จะช่วยเป็นภาพยนตร์เรื่องนั้นเป็นตำนานก็คือตัวเลขรายได้รวมจาก box office ทั่วโลกว่ามีมากน้อยเพียงใด

ในการคำนวณรายได้ทั้งหมดของภาพยนตร์ Star Wars ทั้ง 3 ภาค คุณจะต้องรวมคอลัมน์รายได้จาก US และที่ไม่ใช่ US เข้าด้วยกัน

ใน R function [`rowSums()`](http://www.rdocumentation.org/packages/base/functions/colSums) สามารถช่วยให้เราคำนวณยอดรวมของแต่่ละแถวใน matrix ได้ ซึ่ง function นี้จะทำการสร้าง vector ใหม่:
...
*** =instructions
คำนวณรายได้รวมจาก box office ของภาพยนตร์ทั้ง 3 ภาคแล้วเก็บผลลัพธ์ไว้ใน vector ชื่อ `worldwide_vector`
...
## การเพิ่มคอลัมน์เพื่อเก็บรายได้รวมจาก box office
...
เพื่อให้อะไรๆง่ายขึ้น คุณสามารถเพิ่มคอลัมน์ (1 คอลัมน์หรือมากกว่า) ลงไปใน matrix ได้ด้วย function [`cbind()`](http://www.rdocumentation.org/packages/base/functions/cbind) ซึ่งจะนำ matrix มาต่อกับ matrix หรือ vector อื่นๆได้ ยกตัวอย่างเช่น: 
...
*** =hint
ในแบบฝึกหัดนี้ คุณควรใส่ตัวแปร `star_wars_matrix` และ `worldwide_vector` ลงไปใน function `cbind()` ตามลำดับ จากนั้นให้เก็ผลลัพธ์ไว้ในตัวแปร `all_wars_matrix`
